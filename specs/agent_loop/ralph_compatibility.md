# Agent Runtime Adapter Spec (TASK-NNN 정식안)
...
Agent Runtime 원안의 핵심 파일/흐름을 그대로 복사하지 않고, 현재 운영 체계(`AGENTS.md` + task registry + review/audit 체계)를 source-of-truth로 두는 형태의 adapter 규격을 정식 정의한다.
...
## 1) Agent Runtime 원문 파일 ↔ 현재 소스매핑

| Agent Runtime 대상 | 본 프로젝트 대응 | 구현 포인트 |
|---|---|---|
...
Agent Runtime adapter의 핵심 제약은 "새 파일 이름을 만들 수 있는가"가 아니라 "어느 파일이 최종 상태를 말할 권한을 갖는가"다. 아래 구분을 벗어나면 source-of-truth drift로 본다.

| 분류 | 파일/디렉터리 | 권한 | 금지 |
|---|---|---|---|
| Canonical policy | `AGENTS.md`, `agents/roles.yml`, `agents/*/SKILL.md`, `CLAUDE.md` 보조 절 | 역할·권한·시작 프로토콜·금지 범위 | Agent Runtime 전용 `SYSTEM.md`로 중복 정의 금지 |
...
| repeated same role/task | `REPEATED_FAILURE_THRESHOLD=3` | orchestrator는 warn, Agent Runtime loop는 추가 자동 fan-out 금지 | `SAFETY-*.json` when emitted |
...
Agent Runtime 원안의 `PLAN.md`/`LOG.md`를 이식하지 않는 대신, loop가 매 반복마다 아래 순서를 지켜 drift를 줄인다.
...
TASK-NNN은 새 runner를 만드는 TASK가 아니라, 이미 생긴 runner/message/safety 자산을 Agent Runtime adapter 관점에서 정합화하는 spec freeze 작업이다. 현재 의존성은 아래처럼 닫힌다.
...
| `TASK-NNN` agent message bus | 완료, `TASK-NNN`으로 흡수 | Agent Runtime mailbox 요구는 `agents/messages/` + `check_messages.py`로 충족 | 새 mailbox 없음 |
...
따라서 TASK-NNN 완료 후 즉시 해야 할 일은 "새 파일 세트 생성"이 아니라, 이후 Agent Runtime 관련 변경이 위 canonical 자산을 재사용하도록 하는 것이다.

### Dependency matrix

| TASK | depends_on | blocks | handoff artifact | acceptance signal |
|---|---|---|---|---|
| `TASK-NNN` runner | `TASK-NNN` mapping, `TASK-NNN` mode contract, `TASK-NNN` safety gate | Agent Runtime loop execution changes | `scripts/agent_loop.py`, `scripts/test_agent_loop.py`, event log shape | `agent_loop --help`, mode dry-run/non-dry-run tests, safety caps/dirty-stop tests |
...
- `TASK-NNN~073` are not open dependencies for TASK-NNN. They are already implemented or superseded, and TASK-NNN now records how later Agent Runtime work must reuse them.
- If a later Agent Runtime change needs a new behavior outside this matrix, it must be a new TASK. It must not silently redefine TASK-NNN as an implementation task.

## 6) 이행 제한(옵션 C에서 유지)

- 본 작업은 지금은 **"Agent Runtime을 복제하지 않고 adapter를 정의"**한다.
...
| 기준 | 충족 증거 |
|---|---|
| Agent Runtime 파일 매핑이 존재 | 본 문서 §1 |
