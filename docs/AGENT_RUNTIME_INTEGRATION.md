...
- **틀 = agent_runtime(①, 정본·자동 업데이트).** **맥락 = Autofolio 추가 파일(②, 항상 안전).** **시임 = 2파일(③, `unmanaged` 분리·수동 재조정).**
- 업데이트는 `sync --check → --diff → --apply` 로 받고, 시임은 §4-4 로만 손댄다. Autofolio 튜닝은 **절대 자동으로 덮어써지지 않는다.**
