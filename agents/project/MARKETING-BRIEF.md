...
Related tasks: TASK-092, TASK-093, TASK-095, TASK-096, TASK-097, TASK-128, TASK-129, TASK-130, TASK-131, TASK-132, TASK-133, TASK-134, TASK-135, TASK-136, TASK-137, TASK-138, TASK-166, TASK-167, TASK-168, TASK-169, TASK-170
Related tasksets: TASKSET-MARKETING-GROWTH, TASKSET-MARKETING-TEAM-OPERATING-SYSTEM, TASKSET-MARKETING-ASSET-RENDERING-FOUNDATION, TASKSET-MARKETING-ASSET-CLAIM-REVIEW-FOUNDATION, TASKSET-MARKETING-ASSET-REVIEW-QUEUE-FOUNDATION, TASKSET-MARKETING-ASSET-REVIEW-AUDIT-PREVIEW, TASKSET-MARKETING-ASSET-OWNER-REVIEW-PACKET, TASKSET-MARKETING-ASSET-OWNER-DECISION-QUEUE
...
- `TASK-134` completed the local claim-review matrix slice.
...
`TASK-134` is complete as the no-Owner local claim classification slice for
landing, PDF, PPTX, and SNS asset source claims.
...
- The matrix records source hashes for `PROMOTION-ASSET-PREVIEW-MANIFEST`,
  `MARKETING-MATERIALS-V1`, and `PROMOTION-ASSET-RENDERING-CONTRACT`.
- Claims are classified into allowed draft, needs Compliance review, Owner-only
  before public use, and reject buckets.
- Landing/PDF/PPTX/SNS preview targets stay `classified_draft_not_approved`
...
python -m pytest tests/unit/test_promotion_asset_claim_review_matrix_gate.py -q
```

Boundary:

- No public approval, final legal/tax/securities advice, final PDF/PPTX binary,
  public landing page, SNS upload, customer contact, CRM/payment record,
  external account action, secret, KIS/order/risk/prod, or deploy change.

## TASK-135 Promotion Asset Review Queue Contract

`TASK-135` is complete as the no-Owner local review queue contract slice for
landing, PDF, PPTX, and SNS asset source claims.
...
- The contract records source hashes for `PROMOTION-ASSET-CLAIM-REVIEW-MATRIX`,
  `PROMOTION-ASSET-PREVIEW-MANIFEST`, and
  `PROMOTION-PUBLISHING-STATE-MACHINE`.
- Review queue states are local-only and all have `live_action=false`.
- Landing/PDF/PPTX/SNS queue items require blockers, evidence, assigned role,
  source hash, rollback/archive instruction, and blocked public use/final
  export/publication approval flags.
- `TASKSET-MARKETING-ASSET-REVIEW-QUEUE-FOUNDATION` is complete.
- `TASK-136` completed the local audit-preview slice.

Validation:

```powershell
python scripts/promotion_asset_review_queue_contract_gate.py --check
python -m pytest tests/unit/test_promotion_asset_review_queue_contract_gate.py -q
```
...
- No public approval, final legal/tax/securities advice, final PDF/PPTX binary,
  public landing page, SNS upload, customer contact, CRM/payment record,
  external account action, OAuth, platform API call, secret, KIS/order/risk/
  prod, or deploy change.

## TASK-136 Promotion Asset Review Queue Audit Preview

`TASK-136` is complete as the no-Owner local audit preview slice for the
promotion asset review queue.
...
- The preview records a source hash for
  `PROMOTION-ASSET-REVIEW-QUEUE-CONTRACT`.
- All four landing/PDF/PPTX/SNS queue items are summarized with state, role,
  claim bucket, blocker/evidence count, and blocked public/final/export/live/
  customer/CRM-payment/secret flags.
- The blocked-action scan covers public use, final export, SNS upload, external
  action, customer contact, CRM/payment, secret material, final advice, and
  KIS/order/risk/prod/deploy.
- `TASKSET-MARKETING-ASSET-REVIEW-AUDIT-PREVIEW` is complete.
- `TASK-137` completed the local Owner-review packet slice.

Validation:

```powershell
python scripts/promotion_asset_review_queue_audit_preview_gate.py --check
python -m pytest tests/unit/test_promotion_asset_review_queue_audit_preview_gate.py -q
```
...
`TASK-137` is complete as the no-Owner local Owner review packet slice for the
promotion asset review queue.
...
- The packet records a source hash for
  `PROMOTION-ASSET-REVIEW-QUEUE-AUDIT-PREVIEW`.
- Landing/PDF/PPTX/SNS queue items are packaged as Owner decision required.
- Decision list covers public landing use, final PDF/PPTX export, SNS upload,
  customer contact, CRM/payment setup, paid ads, external account action, and
  legal/tax/securities reliance.
- Evidence map, blocked action list, escalation checklist, and forbidden
  outputs are recorded without approving publication or export.
- `TASKSET-MARKETING-ASSET-OWNER-REVIEW-PACKET` is complete.
- `TASK-138` is registered as the next no-Owner local decision-queue contract
  slice.
...
python -m pytest tests/unit/test_promotion_asset_owner_review_packet_gate.py -q
```

Boundary:

- No actual Owner approval record, public approval, final legal/tax/securities
  advice, final PDF/PPTX binary, public landing page, SNS upload, customer
  contact, CRM/payment record, external account action, OAuth, platform API
  call, secret, KIS/order/risk/prod, or deploy change.
...
- TASK-134: completed local promotion asset claim review matrix.
- TASK-135: completed local promotion asset review queue contract.
- TASK-136: completed local promotion asset review queue audit preview.
- TASK-137: registered as the next no-Owner local Owner review packet slice.
...
- TASKSET-MARKETING-ASSET-CLAIM-REVIEW-FOUNDATION: completed.
- TASKSET-MARKETING-ASSET-REVIEW-QUEUE-FOUNDATION: completed.
- TASKSET-MARKETING-ASSET-REVIEW-AUDIT-PREVIEW: completed.
- TASKSET-MARKETING-ASSET-OWNER-REVIEW-PACKET: active local Owner-review packet
  foundation lane; no public approval, final advice, export, publication,
  customer contact, CRM/payment, external account action, OAuth, or platform
  API action is approved.
