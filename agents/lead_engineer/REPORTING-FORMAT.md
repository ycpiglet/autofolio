...
## Executive BRIEF v2: human-centered and machine-readable
...
| Item | State | Evidence |
|------|-------|----------|
| <핵심 신호> | pass/watch/block | <링크 또는 수치> |
...
- 불릿은 한 줄 중심.
- 표는 4열 이하를 기본값으로.
- 상태는 `pass/watch/block`, 점수는 `score: 0-100`, 우선순위는 `Critical/High/Medium/Low`.
- 제목은 판단 흐름 순서로 고정: `Bottom Line -> Signal -> Insight -> Decision -> Action -> Footer`.
...
- 상태는 `pass/watch/block`(정상/주의/차단)으로. 숫자는 의사결정에 필요한 것만 (비용·일정·리스크·score: 0-100).
...
- 기존 BRIEF/PLAN 형식(Signal 표·Insight·Decision·pass/watch/block) 그대로.
...
운영 규칙:
- Owner/CEO에게는 agent 내부 상태를 그대로 던지지 않고, 사용자 언어를 유지한 의사결정 가능한 `Bottom Line -> Signal -> Insight -> Decision`으로 변환한다.
...
| 숫자 우선 | 형용사보다 `N건`, `%`, `ph`, `tokens`, `score: 0-100`, `pass/watch/block` 상태를 우선한다. |
| 인사이트 포함 | 모든 Standard 이상 보고에는 `Insight` 1~3개를 둔다. |
| 장식 최소 | 장식용 이모지는 쓰지 않는다. O/X/체크박스 같은 단순 상태 마커는 허용 (남발 금지). §이모지 정책 참조. |
| 시각 표현 | 표, ASCII bar, pass/watch/block, delta(`+/-`)를 쓴다. |
...
| `block` | 차단 | 비용/품질/보안 위험 |
| `info` | 정보 | 추세나 참고 데이터 |

### 대화 응답 전 자체 점검

- 사용자 언어를 따른다. 사용자 또는 Owner와 직접 대화할 때는 한국어를 기본값으로 둔다.
- 첫 문장은 `Bottom Line` 성격의 결론이어야 한다.
- 상태는 `pass/watch/block`, 필요 시 `score: 0-100`으로 쓴다.
- 판단 흐름은 `Bottom Line -> Signal -> Insight -> Decision`을 우선한다.
...
```text
Bottom Line: <결과 한 줄>. 상태 <pass/watch/block>, 결정 <없음/1건>.
Metric: <핵심 수치 1~2개>.
Next: <다음 행동 또는 없음>.
```
...
- 단위 표기 고정: 시간 `ph`, 토큰 `~K`, 상태 `pass/watch/block/info`, 증감 `+/-%`.
...
- 2026-05-21 — 효율 재설계 ([TASK-NNN](tasks/TASK-NNN-reporting-format-efficiency.md)). 이모지 기본 제거, 표/숫자/상태 신호/ASCII bar 중심, `Insight` 섹션 필수화, 문서 길이와 샘플 과잉 축소.
