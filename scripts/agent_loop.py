#!/usr/bin/env python3
"""Agent Runtime-style meta-loop runner (TASK-070).
...
def apply_loop_safety_caps(cfg: LoopConfig) -> tuple[LoopConfig, list[str]]:
    """Normalize Agent Runtime loop config so "until done" runs remain bounded.
...
def _run_build_dispatch(cfg: LoopConfig) -> str:
    ...
    Closes the Agent Runtime loop (TASK-213): build mode can now actually drain pending
...
def collab_gate_summary() -> list[str]:
    ...
    Closed-loop step (CYCLE-073): Agent Runtime's review mode now reads cycle_gate so the
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="agent_loop",
        description="Agent Runtime-style meta-loop runner (TASK-070). OS-independent.",
    )
