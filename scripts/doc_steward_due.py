...
않는다. Doc Steward 고유 영역인 **drift(정합성 표류)** 신호만 본다:
  ...
  D2 missing review  : 완료/부분완료된 최신 CYCLE-NNN.md 에 대응하는 REVIEW-NNN.md 가 없음.
...
def cycle_status(cycle_id: int) -> str:
    path = LEAD / f"CYCLE-{cycle_id:03d}.md"
    if not path.exists():
        return ""
...
def missing_review() -> int:
    """완료/부분완료 최신 사이클에 대응하는 REVIEW 가 없으면 그 번호를 반환(없으면 -1).
    ...
    진행 중 CYCLE은 아직 REVIEW 작성 시점이 아니므로 drift 로 세지 않는다.
    """
    ...
    if rev_path.exists():
        return -1
    status = cycle_status(cyc)
    return cyc if "완료" in status else -1
