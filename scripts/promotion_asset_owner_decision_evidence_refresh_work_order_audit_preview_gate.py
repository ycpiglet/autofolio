"""Validate the TASK-147 promotion asset Owner evidence refresh work-order audit preview.
...
platform calls, secret fields, and missing work-order, state, trigger,
precondition/proof/expiry, archive/rollback, or blocked-action coverage.
"""
...
DEFAULT_PREVIEW = REPO_ROOT / "agents" / "project" / "PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-REFRESH-WORK-ORDER-AUDIT-PREVIEW.json"
SOURCE_CONTRACT_PATH = REPO_ROOT / "agents" / "project" / "PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-REFRESH-WORK-ORDER-CONTRACT.json"

REQUIRED_SOURCE_IDS = {"promotion_asset_owner_decision_evidence_refresh_work_order_contract"}
EXPECTED_SOURCE_PATHS = {
    "promotion_asset_owner_decision_evidence_refresh_work_order_contract": "agents/project/PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-REFRESH-WORK-ORDER-CONTRACT.json",
}

REQUIRED_BOUNDARIES = {
    "local_refresh_work_order_audit_readiness_preview_only",
...
REQUIRED_SUMMARY_COUNTS = {
    "decision_types": 9,
    "source_refresh_work_order_records": 9,
    "refresh_work_order_record_summaries": 9,
    "work_order_states": 5,
    "invalidating_trigger_map_items": 8,
    "precondition_coverage_items": 9,
    "proof_requirement_coverage_items": 9,
    "expiry_trigger_coverage_items": 9,
...
def load_preview(path: Path = DEFAULT_PREVIEW) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError("promotion asset Owner decision evidence refresh work-order audit preview root must be an object")
    return data
...
def validate_preview(preview: dict[str, Any], repo_root: Path = REPO_ROOT) -> list[str]:
    ...
    if preview.get("$schema") != "autofolio.promotion-asset-owner-decision-evidence-refresh-work-order-audit-preview/v1":
        findings.append("unexpected or missing promotion asset Owner decision evidence refresh work-order audit preview schema")

    if preview.get("status") != "local_refresh_work_order_audit_readiness_preview_not_actual_refresh_execution":
        findings.append("status must be local_refresh_work_order_audit_readiness_preview_not_actual_refresh_execution")
    ...
    findings.extend(_validate_refresh_work_order_record_summaries(preview))
    findings.extend(_validate_work_order_state_safety_scan(preview))
    findings.extend(_validate_trigger_map_coverage_scan(preview))
    findings.extend(_validate_precondition_proof_expiry_coverage_scan(preview))
...
def _validate_refresh_work_order_record_summaries(preview: dict[str, Any]) -> list[str]:
    summaries = preview.get("refresh_work_order_record_summaries")
    if not isinstance(summaries, list):
        return ["refresh_work_order_record_summaries must be a list"]
    source = _load_json(SOURCE_CONTRACT_PATH)
    source_records = {
        item.get("decision_type"): item
        for item in source.get("refresh_work_order_records", [])
    ...
    if missing:
        findings.append(f"refresh_work_order_record_summaries missing decision types: {sorted(missing)}")
    for index, item in enumerate(summaries):
        if not isinstance(item, dict):
            findings.append(f"refresh_work_order_record_summaries[{index}] must be an object")
            continue
        prefix = f"refresh_work_order_record_summaries[{index}]"
        ...
        for key, source_key in (
            ...
            ("current_work_order_state", "current_work_order_state"),
            ...
            if item.get(key) != source_record.get(source_key):
                findings.append(f"{prefix}.{key} must match source contract {source_key}")
        expected_review_roles = _string_list(source_record.get("review_roles"))
        if item.get("review_roles_count") != len(expected_review_roles):
            findings.append(f"{prefix}.review_roles_count must match source contract review_roles")
        for key, source_key in (
            ("precondition_count", "preconditions"),
            ("proof_requirement_count", "proof_requirements"),
            ("expiry_trigger_count", "expiry_triggers"),
        ):
            if item.get(key) != len(_string_list(source_record.get(source_key))):
                findings.append(f"{prefix}.{key} must match source contract {source_key}")
...
def _validate_work_order_state_safety_scan(preview: dict[str, Any]) -> list[str]:
    scan = preview.get("work_order_state_safety_scan")
    if not isinstance(scan, list):
        return ["work_order_state_safety_scan must be a list"]
    source = _load_json(SOURCE_CONTRACT_PATH)
    source_states = {
        item.get("state_id"): item
        for item in source.get("work_order_states", [])
        if isinstance(item, dict)
    }
    findings: list[str] = []
    missing = set(source_states) - {item.get("state_id") for item in scan if isinstance(item, dict)}
    if missing:
        findings.append(f"work_order_state_safety_scan missing state ids: {sorted(missing)}")
    for index, item in enumerate(scan):
        if not isinstance(item, dict):
            findings.append(f"work_order_state_safety_scan[{index}] must be an object")
            continue
        prefix = f"work_order_state_safety_scan[{index}]"
...
def _validate_trigger_map_coverage_scan(preview: dict[str, Any]) -> list[str]:
    ...
    source_items = {
        ...
        for item in source.get("invalidating_trigger_to_work_order_map", [])
        if isinstance(item, dict)
    }
    findings: list[str] = []
...
def _validate_precondition_proof_expiry_coverage_scan(preview: dict[str, Any]) -> list[str]:
    scan = preview.get("precondition_proof_expiry_coverage_scan")
    if not isinstance(scan, list):
        return ["precondition_proof_expiry_coverage_scan must be a list"]
    source = _load_json(SOURCE_CONTRACT_PATH)
    source_records = {
        item.get("decision_type"): item
        for item in source.get("refresh_work_order_records", [])
    ...
    if missing:
        findings.append(f"precondition_proof_expiry_coverage_scan missing decision types: {sorted(missing)}")
    for index, item in enumerate(scan):
        if not isinstance(item, dict):
            findings.append(f"precondition_proof_expiry_coverage_scan[{index}] must be an object")
            continue
        prefix = f"precondition_proof_expiry_coverage_scan[{index}]"
        source_record = source_records.get(item.get("decision_type"))
        if not isinstance(source_record, dict):
            findings.append(f"{prefix}.decision_type unknown")
            continue
        if item.get("source_work_order_id") != source_record.get("work_order_id"):
            findings.append(f"{prefix}.source_work_order_id must match source contract")
        for key, source_key in (
            ("precondition_count", "preconditions"),
            ("proof_requirement_count", "proof_requirements"),
            ("expiry_trigger_count", "expiry_triggers"),
        ...
        if item.get("coverage_status") != "pass":
            findings.append(f"{prefix}.coverage_status must be pass")
    return findings
...
def _validate_events(preview: dict[str, Any]) -> list[str]:
    ...
    for required in (
        "refresh_work_order_audit_preview_generated",
        "refresh_work_order_record_coverage_scan_recorded",
        "work_order_state_safety_scan_passed",
        "invalidating_trigger_map_coverage_scan_passed",
        "precondition_proof_expiry_coverage_scan_passed",
...
def _validate_handoff(preview: dict[str, Any]) -> list[str]:
    ...
    findings: list[str] = []
    for key in ("task_147_preview_ready", "taskset_local_scope_complete_when_task_closes"):
...
def _validate_verification(preview: dict[str, Any]) -> list[str]:
    ...
    if "promotion_asset_owner_decision_evidence_refresh_work_order_audit_preview_gate.py --check" not in str(verification.get("local_gate", "")):
...
def main() -> int:
    ...
    if findings:
        print("promotion asset Owner decision evidence refresh work-order audit preview gate: FAIL")
        for finding in findings:
            print(f"- {finding}")
        return 1
    print(f"promotion asset Owner decision evidence refresh work-order audit preview gate: PASS ({args.preview})")
    return 0
