"""Validate the TASK-153 Owner/R3 packet review submission preflight audit preview.
...
action, external account action, platform calls, secret fields, and drift from
the source submission preflight contract.
"""
...
REQUIRED_SOURCE_IDS = {
    "promotion_asset_owner_decision_evidence_refresh_owner_r3_packet_review_submission_preflight_contract"
}
...
REQUIRED_BOUNDARIES = {
    "local_owner_r3_packet_review_submission_preflight_audit_readiness_preview_only",
    "submission_preflight_audit_not_submission",
    "submission_preflight_not_submission",
    "submission_preflight_audit_not_approval",
...
REQUIRED_SUMMARY_COUNTS = {
    "decision_types": 9,
    "source_submission_preflight_records": 9,
    "submission_preflight_record_summaries": 9,
    "source_preflight_state_records": 6,
    "preflight_state_safety_scan_items": 6,
    "preflight_prerequisite_summaries": 9,
    "owner_r3_decision_package_input_summaries": 9,
    "submission_blocker_summaries": 9,
...
def load_preview(path: Path = DEFAULT_PREVIEW) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError("promotion asset Owner/R3 packet review submission preflight audit preview root must be an object")
    return data
...
def validate_preview(preview: dict[str, Any], repo_root: Path = REPO_ROOT) -> list[str]:
    ...
    if preview.get("$schema") != (
        "autofolio.promotion-asset-owner-decision-evidence-refresh-owner-r3-packet-review-submission-preflight-audit-preview/v1"
    ):
        findings.append("unexpected or missing promotion asset Owner/R3 packet review submission preflight audit preview schema")

    if preview.get("status") != "local_owner_r3_packet_review_submission_preflight_audit_readiness_preview_not_actual_submission":
        findings.append("status must be local_owner_r3_packet_review_submission_preflight_audit_readiness_preview_not_actual_submission")
    ...
    findings.extend(_validate_summary(preview, source_contract))
    findings.extend(_validate_source_copies(preview, source_contract))
    findings.extend(_validate_submission_preflight_record_summaries(preview))
    findings.extend(_validate_preflight_state_safety_scan(preview))
    findings.extend(_validate_preflight_prerequisite_summaries(preview))
    findings.extend(_validate_owner_r3_decision_package_input_summaries(preview))
    findings.extend(_validate_submission_blocker_summaries(preview))
    findings.extend(_validate_invalidating_trigger_summaries(preview))
    findings.extend(_validate_blocked_action_scan(preview, source_contract))
...
def _validate_markdown_companion(preview: dict[str, Any], repo_root: Path) -> list[str]:
    ...
    findings: list[str] = []
    for required in (
        "submission preflight audit/readiness preview",
        "submission preflight audit not submission",
        "submission preflight audit not approval",
        ...
        "submission preflight record summaries",
        "preflight state safety scan",
        "preflight prerequisite summaries",
        "owner/r3 decision package input summaries",
        "submission blocker summaries",
...
def _validate_summary(preview: dict[str, Any], source_contract: dict[str, Any]) -> list[str]:
    ...
    if summary.get("source_submission_preflight_contract") != source_contract.get("contract_id"):
        findings.append("audit_summary.source_submission_preflight_contract must match source contract_id")
    ...
    for key in (
        ...
        if summary.get(key) != source_contract.get("contract_summary", {}).get(key):
            findings.append(f"audit_summary.{key} must match source contract summary")
    return findings


def _validate_source_copies(preview: dict[str, Any], source_contract: dict[str, Any]) -> list[str]:
    findings: list[str] = []
    for key, source_key in (
        ("submission_preflight_record_summaries", "submission_preflight_records"),
        ("preflight_state_safety_scan", "submission_preflight_state_records"),
        ("preflight_prerequisite_summaries", "submission_preflight_prerequisites"),
        ("owner_r3_decision_package_input_summaries", "owner_r3_decision_package_inputs"),
        ("submission_blocker_summaries", "submission_blocker_records"),
        ("invalidating_trigger_summaries", "invalidating_trigger_map"),
        ...
        if preview.get(key) != source_contract.get(source_key):
            findings.append(f"{key} must match source contract {source_key}")
    return findings


def _validate_submission_preflight_record_summaries(preview: dict[str, Any]) -> list[str]:
    records = preview.get("submission_preflight_record_summaries")
    if not isinstance(records, list):
        return ["submission_preflight_record_summaries must be a list"]
    findings: list[str] = []
    missing = REQUIRED_DECISION_TYPES - {item.get("decision_type") for item in records if isinstance(item, dict)}
    if missing:
        findings.append(f"submission_preflight_record_summaries missing decision types: {sorted(missing)}")
    for index, item in enumerate(records):
        if not isinstance(item, dict):
            findings.append(f"submission_preflight_record_summaries[{index}] must be an object")
            continue
        prefix = f"submission_preflight_record_summaries[{index}]"
...
def _validate_preflight_state_safety_scan(preview: dict[str, Any]) -> list[str]:
    states = preview.get("preflight_state_safety_scan")
    if not isinstance(states, list):
        return ["preflight_state_safety_scan must be a list"]
    findings: list[str] = []
    missing = REQUIRED_PREFLIGHT_STATE_IDS - {item.get("state_id") for item in states if isinstance(item, dict)}
    if missing:
        findings.append(f"preflight_state_safety_scan missing state ids: {sorted(missing)}")
    for index, item in enumerate(states):
        if not isinstance(item, dict):
            findings.append(f"preflight_state_safety_scan[{index}] must be an object")
            continue
        prefix = f"preflight_state_safety_scan[{index}]"
        for key in (
            "live_action",
            "owner_r3_review_submission_allowed",
            "actual_owner_r3_review_submitted",
...
def _validate_preflight_prerequisite_summaries(preview: dict[str, Any]) -> list[str]:
    records = preview.get("preflight_prerequisite_summaries")
    if not isinstance(records, list):
        return ["preflight_prerequisite_summaries must be a list"]
    findings: list[str] = []
    missing = REQUIRED_DECISION_TYPES - {item.get("decision_type") for item in records if isinstance(item, dict)}
    if missing:
        findings.append(f"preflight_prerequisite_summaries missing decision types: {sorted(missing)}")
    for index, item in enumerate(records):
        if not isinstance(item, dict):
            findings.append(f"preflight_prerequisite_summaries[{index}] must be an object")
            continue
        prefix = f"preflight_prerequisite_summaries[{index}]"
...
def _validate_owner_r3_decision_package_input_summaries(preview: dict[str, Any]) -> list[str]:
    records = preview.get("owner_r3_decision_package_input_summaries")
    if not isinstance(records, list):
        return ["owner_r3_decision_package_input_summaries must be a list"]
    findings: list[str] = []
    missing = REQUIRED_DECISION_TYPES - {item.get("decision_type") for item in records if isinstance(item, dict)}
    if missing:
        findings.append(f"owner_r3_decision_package_input_summaries missing decision types: {sorted(missing)}")
    for index, item in enumerate(records):
        if not isinstance(item, dict):
            findings.append(f"owner_r3_decision_package_input_summaries[{index}] must be an object")
            continue
        prefix = f"owner_r3_decision_package_input_summaries[{index}]"
...
def _validate_submission_blocker_summaries(preview: dict[str, Any]) -> list[str]:
    records = preview.get("submission_blocker_summaries")
    if not isinstance(records, list):
        return ["submission_blocker_summaries must be a list"]
    findings: list[str] = []
    missing = REQUIRED_DECISION_TYPES - {item.get("decision_type") for item in records if isinstance(item, dict)}
    if missing:
        findings.append(f"submission_blocker_summaries missing decision types: {sorted(missing)}")
    for index, item in enumerate(records):
        if not isinstance(item, dict):
            findings.append(f"submission_blocker_summaries[{index}] must be an object")
            continue
        prefix = f"submission_blocker_summaries[{index}]"
...
def _validate_invalidating_trigger_summaries(preview: dict[str, Any]) -> list[str]:
    records = preview.get("invalidating_trigger_summaries")
    if not isinstance(records, list):
        return ["invalidating_trigger_summaries must be a list"]
    findings: list[str] = []
    missing = REQUIRED_DECISION_TYPES - {item.get("decision_type") for item in records if isinstance(item, dict)}
    if missing:
        findings.append(f"invalidating_trigger_summaries missing decision types: {sorted(missing)}")
    for index, item in enumerate(records):
        if not isinstance(item, dict):
...
def _validate_blocked_action_scan(preview: dict[str, Any], source_contract: dict[str, Any]) -> list[str]:
    scan = preview.get("blocked_action_scan")
    if not isinstance(scan, dict):
        return ["blocked_action_scan must be an object"]
    source_scan = source_contract.get("blocked_action_scan")
...
def _validate_events(preview: dict[str, Any]) -> list[str]:
    ...
    for required in (
        "submission_preflight_audit_preview_generated",
        "source_submission_preflight_contract_hash_recorded",
        "submission_preflight_record_summaries_recorded",
        "preflight_state_safety_scan_recorded",
        "preflight_prerequisite_coverage_recorded",
        "owner_r3_decision_package_input_coverage_recorded",
        "submission_blocker_coverage_recorded",
...
def _validate_handoff(preview: dict[str, Any]) -> list[str]:
    ...
    findings: list[str] = []
    for key in ("task_153_preview_ready", "taskset_local_scope_complete_when_task_closes"):
        if handoff.get(key) is not True:
            findings.append(f"taskset_handoff.{key} must be true")
    if "handoff packet candidate" not in str(handoff.get("next_allowed_no_owner_slice", "")).lower():
        findings.append("taskset_handoff.next_allowed_no_owner_slice must describe handoff packet candidate")
...
def _validate_verification(preview: dict[str, Any]) -> list[str]:
    ...
    if (
        "promotion_asset_owner_decision_evidence_refresh_owner_r3_packet_review_submission_preflight_audit_preview_gate.py --check"
...
def main() -> int:
    ...
    if findings:
        print("promotion asset Owner/R3 packet review submission preflight audit preview gate: FAIL")
        for finding in findings:
            print(f"- {finding}")
        return 1
    print(f"promotion asset Owner/R3 packet review submission preflight audit preview gate: PASS ({args.preview})")
    return 0
