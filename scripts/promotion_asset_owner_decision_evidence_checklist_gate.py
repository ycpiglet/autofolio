"""Validate the TASK-140 promotion asset Owner decision evidence checklist.

This gate validates local checklist metadata only. It rejects actual Owner
...
DEFAULT_CONTRACT = REPO_ROOT / "agents" / "project" / "PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-CHECKLIST-CONTRACT.json"
SOURCE_PREVIEW_PATH = REPO_ROOT / "agents" / "project" / "PROMOTION-ASSET-OWNER-DECISION-QUEUE-AUDIT-PREVIEW.json"
EXPECTED_SOURCE_PREVIEW_ID = "PROMOTION-ASSET-OWNER-DECISION-QUEUE-AUDIT-PREVIEW-2026-06-20-001"

REQUIRED_SOURCE_IDS = {"promotion_asset_owner_decision_queue_audit_preview"}
EXPECTED_SOURCE_PATHS = {
    "promotion_asset_owner_decision_queue_audit_preview": "agents/project/PROMOTION-ASSET-OWNER-DECISION-QUEUE-AUDIT-PREVIEW.json",
}

REQUIRED_BOUNDARIES = {
    "local_evidence_checklist_contract_only",
...
REQUIRED_CONTRACT_FORBIDDEN_FIELDS = {
    "approval_signature",
    "approved_by_owner",
...
def load_contract(path: Path = DEFAULT_CONTRACT) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError("promotion asset Owner decision evidence checklist contract root must be an object")
    return data
...
def validate_contract(contract: dict[str, Any], repo_root: Path = REPO_ROOT) -> list[str]:
    ...
    if contract.get("$schema") != "autofolio.promotion-asset-owner-decision-evidence-checklist/v1":
        findings.append("unexpected or missing promotion asset Owner decision evidence checklist schema")

    if contract.get("status") != "local_evidence_checklist_contract_not_approval_evidence":
        findings.append("status must be local_evidence_checklist_contract_not_approval_evidence")
    ...
    findings.extend(_validate_contract_fields(contract))
    findings.extend(_validate_checklist_items(contract))
...
def _validate_summary(contract: dict[str, Any]) -> list[str]:
    summary = contract.get("checklist_summary")
    if not isinstance(summary, dict):
        return ["checklist_summary must be an object"]
    findings: list[str] = []
    for key, expected in REQUIRED_SUMMARY_COUNTS.items():
        if summary.get(key) != expected:
            findings.append(f"checklist_summary.{key} must be {expected}")
    if summary.get("gate_result") != "pass":
        findings.append("checklist_summary.gate_result must be pass")
    return findings


def _validate_contract_fields(contract: dict[str, Any]) -> list[str]:
    checklist_contract = contract.get("checklist_contract")
    if not isinstance(checklist_contract, dict):
        return ["checklist_contract must be an object"]
    findings: list[str] = []
    required_fields = set(_string_list(checklist_contract.get("required_fields")))
    missing_fields = REQUIRED_RECORD_FIELDS - required_fields
    if missing_fields:
        findings.append(f"checklist_contract.required_fields missing: {sorted(missing_fields)}")
    forbidden_fields = set(_string_list(checklist_contract.get("forbidden_fields")))
    missing_forbidden = REQUIRED_CONTRACT_FORBIDDEN_FIELDS - forbidden_fields
    if missing_forbidden:
        findings.append(f"checklist_contract.forbidden_fields missing: {sorted(missing_forbidden)}")
    return findings


def _validate_checklist_items(contract: dict[str, Any]) -> list[str]:
    items = contract.get("checklist_items")
    if not isinstance(items, list):
        return ["checklist_items must be a list"]
    source_preview = _load_json(SOURCE_PREVIEW_PATH)
    evidence_counts = {
        item.get("decision_type"): item.get("required_evidence_count")
        for item in source_preview.get("evidence_gap_scan", [])
        if isinstance(item, dict)
    }
    source_hash = _sha256(SOURCE_PREVIEW_PATH)
    ids = {
    ...
    if missing:
        findings.append(f"checklist_items missing decision types: {sorted(missing)}")
    for index, item in enumerate(items):
        if not isinstance(item, dict):
            findings.append(f"checklist_items[{index}] must be an object")
            continue
        prefix = f"checklist_items[{index}]"
        missing_fields = REQUIRED_RECORD_FIELDS - set(item)
        if missing_fields:
            findings.append(f"{prefix} missing required fields: {sorted(missing_fields)}")
            continue
        decision_type = item.get("decision_type")
        if decision_type not in REQUIRED_DECISION_TYPES:
            findings.append(f"{prefix}.decision_type unknown")
            continue
        if item.get("source_preview_id") != EXPECTED_SOURCE_PREVIEW_ID:
            findings.append(f"{prefix}.source_preview_id must be {EXPECTED_SOURCE_PREVIEW_ID}")
        if str(item.get("source_hash", "")).lower() != source_hash:
            findings.append(f"{prefix}.source_hash must match source preview hash")
        required_evidence = _string_list(item.get("required_evidence"))
        if not required_evidence:
            findings.append(f"{prefix}.required_evidence must be non-empty")
        if len(required_evidence) != evidence_counts.get(decision_type):
            findings.append(f"{prefix}.required_evidence must match source evidence gap count")
        if not isinstance(item.get("accountable_role"), str) or not item.get("accountable_role"):
            findings.append(f"{prefix}.accountable_role must be a non-empty string")
        if not _string_list(item.get("review_roles")):
            findings.append(f"{prefix}.review_roles must be non-empty")
        if not _string_list(item.get("acceptance_criteria")):
            findings.append(f"{prefix}.acceptance_criteria must be non-empty")
        if not _string_list(item.get("stale_evidence_triggers")):
            findings.append(f"{prefix}.stale_evidence_triggers must be non-empty")
        if not _string_list(item.get("forbidden_fields")):
            findings.append(f"{prefix}.forbidden_fields must be non-empty")
        ...
        if not isinstance(item.get("rollback_or_archive_instruction"), str) or not item.get("rollback_or_archive_instruction"):
            findings.append(f"{prefix}.rollback_or_archive_instruction must be a non-empty string")
    return findings
...
def _validate_handoff(contract: dict[str, Any]) -> list[str]:
    ...
    findings: list[str] = []
    for key in ("task_140_checklist_ready", "taskset_local_scope_complete_when_task_closes"):
...
def _validate_verification(contract: dict[str, Any]) -> list[str]:
    ...
    if "promotion_asset_owner_decision_evidence_checklist_gate.py --check" not in str(verification.get("local_gate", "")):
...
def main() -> int:
    ...
    if findings:
        print("promotion asset Owner decision evidence checklist gate: FAIL")
        for finding in findings:
            print(f"- {finding}")
        return 1
    print(f"promotion asset Owner decision evidence checklist gate: PASS ({args.contract})")
    return 0
