...
import scripts.agent_runtime_migration_inventory as rmi
...
def test_host_state_paths_do_not_classify_as_core():
    ...
    assert _class("agent_runtime.yml") == "host-state"
...
def test_reusable_automation_classifies_as_core_or_template():
    ...
    assert _class("scripts/agent_runtime_migration_inventory.py") == "core"
    assert _class("scripts/test_agent_runtime_migration_inventory.py") == "core"
...
def test_render_outputs_counts_and_safety_line():
    ...
    assert "# Agent Runtime Migration Inventory" in report
