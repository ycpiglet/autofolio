...
The repeat problem: the role machinery (subagent dispatch, retro, seminar, Agent Runtime,
...
def main() -> int:
    ...
    lines.append("  • 자가개선 API: /retro(agent_retro run --all) · /seminar(agent_seminar, T2/T3 결정) · "
                 "council(subagent_council) · feed-forward(promote_retro_forward) · Agent Runtime(agent_loop)")
