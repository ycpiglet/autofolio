"""Validate the TASK-141 promotion asset Owner evidence checklist audit preview.
...
DEFAULT_PREVIEW = REPO_ROOT / "agents" / "project" / "PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-CHECKLIST-AUDIT-PREVIEW.json"
SOURCE_CONTRACT_PATH = REPO_ROOT / "agents" / "project" / "PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-CHECKLIST-CONTRACT.json"

REQUIRED_SOURCE_IDS = {"promotion_asset_owner_decision_evidence_checklist_contract"}
EXPECTED_SOURCE_PATHS = {
    "promotion_asset_owner_decision_evidence_checklist_contract": "agents/project/PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-CHECKLIST-CONTRACT.json",
}
...
REQUIRED_SUMMARY_COUNTS = {
    ...
    "total_acceptance_criteria_items": 27,
    "total_stale_evidence_trigger_items": 27,
    "total_forbidden_field_items": 36,
    "items_with_acceptance_criteria": 9,
    "items_with_stale_triggers": 9,
    "items_with_forbidden_fields": 9,
    ...
    "stale_evidence_uncovered_records": 0,
}
...
def load_preview(path: Path = DEFAULT_PREVIEW) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError("promotion asset Owner decision evidence checklist audit preview root must be an object")
    return data
...
def validate_preview(preview: dict[str, Any], repo_root: Path = REPO_ROOT) -> list[str]:
    ...
    if preview.get("$schema") != "autofolio.promotion-asset-owner-decision-evidence-checklist-audit-preview/v1":
        findings.append("unexpected or missing promotion asset Owner decision evidence checklist audit preview schema")

    if preview.get("status") != "local_audit_readiness_preview_not_actual_approval_evidence":
        findings.append("status must be local_audit_readiness_preview_not_actual_approval_evidence")
    ...
    findings.extend(_validate_checklist_item_summaries(preview))
    findings.extend(_validate_evidence_alignment_scan(preview))
...
def _validate_checklist_item_summaries(preview: dict[str, Any]) -> list[str]:
    summaries = preview.get("checklist_item_summaries")
    if not isinstance(summaries, list):
        return ["checklist_item_summaries must be a list"]
    source = _load_json(SOURCE_CONTRACT_PATH)
    source_items = {
        item.get("checklist_item_id"): item
        for item in source.get("checklist_items", [])
        if isinstance(item, dict)
    }
    summary_ids = {
        item.get("checklist_item_id")
        for item in summaries
        if isinstance(item, dict) and isinstance(item.get("checklist_item_id"), str)
    }
    findings: list[str] = []
    missing = set(source_items) - summary_ids
    if missing:
        findings.append(f"checklist_item_summaries missing checklist items: {sorted(missing)}")
    for index, item in enumerate(summaries):
        if not isinstance(item, dict):
            findings.append(f"checklist_item_summaries[{index}] must be an object")
            continue
        prefix = f"checklist_item_summaries[{index}]"
        source_item = source_items.get(item.get("checklist_item_id"))
        if not isinstance(source_item, dict):
            findings.append(f"{prefix}.checklist_item_id not found in source contract")
            continue
        for key in ("decision_type", "accountable_role"):
            if item.get(key) != source_item.get(key):
                findings.append(f"{prefix}.{key} must match source contract")
        if item.get("required_evidence_count") != len(source_item.get("required_evidence", [])):
            findings.append(f"{prefix}.required_evidence_count must match source checklist")
        if item.get("acceptance_criteria_count") != len(source_item.get("acceptance_criteria", [])):
            findings.append(f"{prefix}.acceptance_criteria_count must match source checklist")
        if item.get("stale_evidence_trigger_count") != len(source_item.get("stale_evidence_triggers", [])):
            findings.append(f"{prefix}.stale_evidence_trigger_count must match source checklist")
        if item.get("forbidden_field_count") != len(source_item.get("forbidden_fields", [])):
            findings.append(f"{prefix}.forbidden_field_count must match source checklist")
        if item.get("review_role_count") != len(source_item.get("review_roles", [])):
            findings.append(f"{prefix}.review_role_count must match source checklist")
        if item.get("readiness_status") != "local_checklist_ready_requires_future_owner_r3":
            findings.append(f"{prefix}.readiness_status must be local_checklist_ready_requires_future_owner_r3")
...
def _validate_evidence_alignment_scan(preview: dict[str, Any]) -> list[str]:
    scan = preview.get("evidence_alignment_scan")
    if not isinstance(scan, list):
        return ["evidence_alignment_scan must be a list"]
    source = _load_json(SOURCE_CONTRACT_PATH)
    source_items = {
    ...
    findings: list[str] = []
    missing = REQUIRED_DECISION_TYPES - decision_types
    if missing:
        findings.append(f"evidence_alignment_scan missing decision types: {sorted(missing)}")
    for index, item in enumerate(scan):
        if not isinstance(item, dict):
            findings.append(f"evidence_alignment_scan[{index}] must be an object")
            continue
        prefix = f"evidence_alignment_scan[{index}]"
        source_item = source_items.get(item.get("decision_type"))
        if not isinstance(source_item, dict):
            findings.append(f"{prefix}.decision_type unknown")
            continue
        expected_count = len(source_item.get("required_evidence", []))
        if item.get("checklist_item_id") != source_item.get("checklist_item_id"):
            findings.append(f"{prefix}.checklist_item_id must match source checklist")
        if item.get("required_evidence_count") != expected_count:
            findings.append(f"{prefix}.required_evidence_count must match source checklist")
        if item.get("source_required_evidence_count") != expected_count:
            findings.append(f"{prefix}.source_required_evidence_count must match source checklist")
        if item.get("count_alignment_status") != "pass":
            findings.append(f"{prefix}.count_alignment_status must be pass")
        if item.get("acceptance_criteria_status") != "covered":
            findings.append(f"{prefix}.acceptance_criteria_status must be covered")
        if item.get("stale_evidence_trigger_count") != len(source_item.get("stale_evidence_triggers", [])):
            findings.append(f"{prefix}.stale_evidence_trigger_count must match source checklist")
        if item.get("stale_evidence_status") != "covered":
            findings.append(f"{prefix}.stale_evidence_status must be covered")
        if item.get("forbidden_field_status") != "covered":
            findings.append(f"{prefix}.forbidden_field_status must be covered")
        if item.get("status") != "local_evidence_checklist_ready_not_approval_evidence":
            findings.append(f"{prefix}.status must be local_evidence_checklist_ready_not_approval_evidence")
    return findings
...
def _validate_events(preview: dict[str, Any]) -> list[str]:
    ...
    for required in (
        "evidence_checklist_audit_preview_generated",
        "checklist_coverage_scan_recorded",
        "stale_evidence_trigger_scan_passed",
...
def _validate_handoff(preview: dict[str, Any]) -> list[str]:
    ...
    findings: list[str] = []
    for key in ("task_141_preview_ready", "taskset_local_scope_complete_when_task_closes"):
...
def _validate_verification(preview: dict[str, Any]) -> list[str]:
    ...
    if "promotion_asset_owner_decision_evidence_checklist_audit_preview_gate.py --check" not in str(verification.get("local_gate", "")):
...
def main() -> int:
    ...
    if findings:
        print("promotion asset Owner decision evidence checklist audit preview gate: FAIL")
        for finding in findings:
            print(f"- {finding}")
        return 1
    print(f"promotion asset Owner decision evidence checklist audit preview gate: PASS ({args.preview})")
    return 0
