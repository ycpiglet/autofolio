"""Validate the TASK-148 promotion asset Owner/R3 packet candidate contract.

This gate validates local packet-candidate metadata only. It rejects actual
...
platform calls, secret fields, and missing packet-candidate, evidence-bundle,
Owner decision prompt, unresolved-blocker, or blocked-action coverage.
"""
...
DEFAULT_CONTRACT = REPO_ROOT / "agents" / "project" / "PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-REFRESH-OWNER-R3-PACKET-CANDIDATE.json"
SOURCE_PREVIEW_PATH = REPO_ROOT / "agents" / "project" / "PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-REFRESH-WORK-ORDER-AUDIT-PREVIEW.json"

REQUIRED_SOURCE_IDS = {"promotion_asset_owner_decision_evidence_refresh_work_order_audit_preview"}
EXPECTED_SOURCE_PATHS = {
    "promotion_asset_owner_decision_evidence_refresh_work_order_audit_preview": "agents/project/PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-REFRESH-WORK-ORDER-AUDIT-PREVIEW.json",
}

REQUIRED_BOUNDARIES = {
    "local_owner_r3_packet_candidate_contract_only",
...
REQUIRED_SUMMARY_COUNTS = {
    "decision_types": 9,
    "source_work_order_audit_record_summaries": 9,
    "packet_candidate_records": 9,
    "evidence_bundle_references": 9,
    "owner_decision_prompt_items": 9,
    "unresolved_blocker_records": 9,
    "source_work_order_states": 5,
    "source_invalidating_trigger_map_items": 8,
    "source_precondition_coverage_items": 9,
...
def load_contract(path: Path = DEFAULT_CONTRACT) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError("promotion asset Owner/R3 packet candidate contract root must be an object")
    return data
...
def validate_contract(contract: dict[str, Any], repo_root: Path = REPO_ROOT) -> list[str]:
    ...
    if contract.get("$schema") != "autofolio.promotion-asset-owner-decision-evidence-refresh-owner-r3-packet-candidate/v1":
        findings.append("unexpected or missing promotion asset Owner/R3 packet candidate contract schema")

    if contract.get("status") != "local_owner_r3_packet_candidate_contract_not_actual_approval":
        findings.append("status must be local_owner_r3_packet_candidate_contract_not_actual_approval")
    ...
    if forbidden_keys:
        findings.append(f"forbidden refresh/approval/signature/export/customer/secret/final-advice key names present: {forbidden_keys}")

    live_true_paths = _find_live_true_keys(contract)
    if live_true_paths:
        findings.append(f"refresh/approval/signature/public/export/customer/payment/platform/final-advice/live flags must not be true: {live_true_paths}")
    ...
    findings.extend(_validate_packet_candidate_records(contract))
    findings.extend(_validate_evidence_bundle_references(contract))
    findings.extend(_validate_owner_decision_prompts(contract))
    findings.extend(_validate_unresolved_blockers(contract))
    findings.extend(_validate_state_references(contract))
    findings.extend(_validate_trigger_references(contract))
...
def _validate_packet_candidate_records(contract: dict[str, Any]) -> list[str]:
    records = contract.get("packet_candidate_records")
    if not isinstance(records, list):
        return ["packet_candidate_records must be a list"]
    source = _load_json(SOURCE_PREVIEW_PATH)
    source_records = {
        item.get("decision_type"): item
        for item in source.get("refresh_work_order_record_summaries", [])
        if isinstance(item, dict)
    }
    findings: list[str] = []
    missing = REQUIRED_DECISION_TYPES - {item.get("decision_type") for item in records if isinstance(item, dict)}
    if missing:
        findings.append(f"packet_candidate_records missing decision types: {sorted(missing)}")
    for index, item in enumerate(records):
        if not isinstance(item, dict):
            findings.append(f"packet_candidate_records[{index}] must be an object")
            continue
        prefix = f"packet_candidate_records[{index}]"
        source_record = source_records.get(item.get("decision_type"))
        if not isinstance(source_record, dict):
            findings.append(f"{prefix}.decision_type unknown")
            continue
        if item.get("packet_candidate_id") != f"owner-r3-packet-candidate-{_slug(str(item.get('decision_type')))}":
            findings.append(f"{prefix}.packet_candidate_id must derive from decision_type")
        for key, source_key in (
            ("source_work_order_id", "source_work_order_id"),
            ("source_queue_record_id", "source_queue_record_id"),
            ("source_record_id", "source_record_id"),
            ("source_current_work_order_state", "current_work_order_state"),
            ...
            ("expiry_trigger_count", "expiry_trigger_count"),
            ("archive_rollback_required", "archive_rollback_required"),
            ("source_work_order_status", "work_order_status"),
        ):
            if item.get(key) != source_record.get(source_key):
                findings.append(f"{prefix}.{key} must match source preview {source_key}")
        if item.get("candidate_status") != "candidate_only_not_submitted":
            findings.append(f"{prefix}.candidate_status must be candidate_only_not_submitted")
...
def _validate_evidence_bundle_references(contract: dict[str, Any]) -> list[str]:
    refs = contract.get("evidence_bundle_references")
    if not isinstance(refs, list):
        return ["evidence_bundle_references must be a list"]
    source = _load_json(SOURCE_PREVIEW_PATH)
    source_records = {
        item.get("decision_type"): item
        for item in source.get("refresh_work_order_record_summaries", [])
    ...
    findings: list[str] = []
    missing = REQUIRED_DECISION_TYPES - {item.get("decision_type") for item in refs if isinstance(item, dict)}
    if missing:
        findings.append(f"evidence_bundle_references missing decision types: {sorted(missing)}")
    for index, item in enumerate(refs):
        if not isinstance(item, dict):
            findings.append(f"evidence_bundle_references[{index}] must be an object")
            continue
        ...
        if not isinstance(source_record, dict):
            findings.append(f"{prefix}.decision_type unknown")
            continue
        if item.get("evidence_bundle_reference_id") != f"evidence-bundle-{_slug(str(item.get('decision_type')))}":
            findings.append(f"{prefix}.evidence_bundle_reference_id must derive from decision_type")
        for key, source_key in (
            ("source_work_order_id", "source_work_order_id"),
            ("required_evidence_count", "source_required_evidence_count"),
            ("stale_evidence_trigger_count", "source_stale_evidence_trigger_count"),
            ("invalidating_event_count", "source_invalidating_event_count"),
            ("precondition_count", "precondition_count"),
            ("proof_requirement_count", "proof_requirement_count"),
            ("expiry_trigger_count", "expiry_trigger_count"),
            ("archive_rollback_required", "archive_rollback_required"),
            ...
            if item.get(key) != source_record.get(source_key):
                findings.append(f"{prefix}.{key} must match source preview {source_key}")
        if item.get("source_hash_required") is not True:
            findings.append(f"{prefix}.source_hash_required must be true")
...
def _validate_owner_decision_prompts(contract: dict[str, Any]) -> list[str]:
    prompts = contract.get("owner_decision_prompt_map")
    if not isinstance(prompts, list):
        return ["owner_decision_prompt_map must be a list"]
    findings: list[str] = []
    missing = REQUIRED_DECISION_TYPES - {item.get("decision_type") for item in prompts if isinstance(item, dict)}
    if missing:
        findings.append(f"owner_decision_prompt_map missing decision types: {sorted(missing)}")
    for index, item in enumerate(prompts):
        if not isinstance(item, dict):
            findings.append(f"owner_decision_prompt_map[{index}] must be an object")
            continue
        prefix = f"owner_decision_prompt_map[{index}]"
        decision_type = str(item.get("decision_type", ""))
        if decision_type not in REQUIRED_DECISION_TYPES:
            findings.append(f"{prefix}.decision_type unknown")
            continue
        if item.get("owner_decision_prompt_id") != f"owner-decision-prompt-{_slug(decision_type)}":
            findings.append(f"{prefix}.owner_decision_prompt_id must derive from decision_type")
        if item.get("decision_required") is not True:
            findings.append(f"{prefix}.decision_required must be true")
        if item.get("candidate_is_approval") is not False:
            findings.append(f"{prefix}.candidate_is_approval must be false")
        if item.get("actual_owner_approval_recorded") is not False:
            findings.append(f"{prefix}.actual_owner_approval_recorded must be false")
        if item.get("actual_owner_signature_collected") is not False:
            findings.append(f"{prefix}.actual_owner_signature_collected must be false")
        if item.get("public_use_approved") is not False:
            findings.append(f"{prefix}.public_use_approved must be false")
        if item.get("action_permitted_now") is not False:
            findings.append(f"{prefix}.action_permitted_now must be false")
        if item.get("professional_review_required") is not True:
            findings.append(f"{prefix}.professional_review_required must be true")
...
def _validate_unresolved_blockers(contract: dict[str, Any]) -> list[str]:
    blockers = contract.get("unresolved_blocker_map")
    if not isinstance(blockers, list):
        return ["unresolved_blocker_map must be a list"]
    findings: list[str] = []
    missing = REQUIRED_DECISION_TYPES - {item.get("decision_type") for item in blockers if isinstance(item, dict)}
    if missing:
        findings.append(f"unresolved_blocker_map missing decision types: {sorted(missing)}")
    for index, item in enumerate(blockers):
        if not isinstance(item, dict):
            findings.append(f"unresolved_blocker_map[{index}] must be an object")
            continue
        prefix = f"unresolved_blocker_map[{index}]"
        decision_type = str(item.get("decision_type", ""))
        if decision_type not in REQUIRED_DECISION_TYPES:
            findings.append(f"{prefix}.decision_type unknown")
            continue
        if item.get("blocker_id") != f"blockers-{_slug(decision_type)}":
            findings.append(f"{prefix}.blocker_id must derive from decision_type")
        blockers_list = _string_list(item.get("blockers"))
        if len(blockers_list) < 4:
            findings.append(f"{prefix}.blockers must include at least four unresolved blockers")
        if item.get("unresolved_blocker_count") != len(blockers_list):
            findings.append(f"{prefix}.unresolved_blocker_count must match blockers")
        if item.get("owner_r3_required") is not True:
            findings.append(f"{prefix}.owner_r3_required must be true")
        if item.get("action_permitted_now") is not False:
            findings.append(f"{prefix}.action_permitted_now must be false")
        if item.get("public_use_blocked") is not True:
            findings.append(f"{prefix}.public_use_blocked must be true")
        if item.get("final_export_blocked") is not True:
            findings.append(f"{prefix}.final_export_blocked must be true")
        if item.get("status") != "blocked_pending_owner_r3":
            findings.append(f"{prefix}.status must be blocked_pending_owner_r3")
    return findings


def _validate_state_references(contract: dict[str, Any]) -> list[str]:
    refs = contract.get("source_state_safety_references")
    if not isinstance(refs, list):
        return ["source_state_safety_references must be a list"]
    source = _load_json(SOURCE_PREVIEW_PATH)
    source_states = {
        item.get("state_id"): item
        for item in source.get("work_order_state_safety_scan", [])
        if isinstance(item, dict)
    }
    findings: list[str] = []
    missing = set(source_states) - {item.get("state_id") for item in refs if isinstance(item, dict)}
    if missing:
        findings.append(f"source_state_safety_references missing state ids: {sorted(missing)}")
    for index, item in enumerate(refs):
        if not isinstance(item, dict):
            findings.append(f"source_state_safety_references[{index}] must be an object")
            continue
        prefix = f"source_state_safety_references[{index}]"
...
def _validate_trigger_references(contract: dict[str, Any]) -> list[str]:
    refs = contract.get("source_trigger_references")
    if not isinstance(refs, list):
        return ["source_trigger_references must be a list"]
    source = _load_json(SOURCE_PREVIEW_PATH)
    source_items = {
    ...
    findings: list[str] = []
    missing = set(source_items) - {item.get("trigger_id") for item in refs if isinstance(item, dict)}
    if missing:
        findings.append(f"source_trigger_references missing trigger ids: {sorted(missing)}")
    for index, item in enumerate(refs):
        if not isinstance(item, dict):
            findings.append(f"source_trigger_references[{index}] must be an object")
            continue
        prefix = f"source_trigger_references[{index}]"
        source_item = source_items.get(item.get("trigger_id"))
        if not isinstance(source_item, dict):
            findings.append(f"{prefix}.trigger_id unknown")
            continue
        for key in ("target_work_order_state", "archive_required", "refresh_execution_allowed", "actual_refresh_executed", "action_permitted_now", "owner_r3_required_before_action", "coverage_status"):
        ...
        if item.get("refresh_execution_allowed") is not False:
            findings.append(f"{prefix}.refresh_execution_allowed must be false")
        if item.get("actual_refresh_executed") is not False:
            findings.append(f"{prefix}.actual_refresh_executed must be false")
        if item.get("action_permitted_now") is not False:
            findings.append(f"{prefix}.action_permitted_now must be false")
        if item.get("status") != "pass":
            findings.append(f"{prefix}.status must be pass")
...
def _validate_events(contract: dict[str, Any]) -> list[str]:
    ...
    for required in (
        "owner_r3_packet_candidate_contract_generated",
        "packet_candidate_records_seeded",
        "evidence_bundle_references_recorded",
        "owner_decision_prompt_map_recorded",
        "unresolved_blocker_map_recorded",
...
def _validate_handoff(contract: dict[str, Any]) -> list[str]:
    ...
    findings: list[str] = []
    for key in ("task_148_contract_ready", "taskset_local_scope_complete_when_task_closes"):
...
def _validate_verification(contract: dict[str, Any]) -> list[str]:
    ...
    if "promotion_asset_owner_decision_evidence_refresh_owner_r3_packet_candidate_gate.py --check" not in str(verification.get("local_gate", "")):
...
def main() -> int:
    ...
    if findings:
        print("promotion asset Owner/R3 packet candidate contract gate: FAIL")
        for finding in findings:
            print(f"- {finding}")
        return 1
    print(f"promotion asset Owner/R3 packet candidate contract gate: PASS ({args.contract})")
    return 0
