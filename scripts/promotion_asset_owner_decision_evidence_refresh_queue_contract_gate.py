"""Validate the TASK-144 promotion asset Owner evidence refresh queue contract.

This gate validates local refresh queue contract metadata only. It rejects
...
missing queue, trigger, state, or blocked-action coverage.
"""
...
DEFAULT_CONTRACT = REPO_ROOT / "agents" / "project" / "PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-REFRESH-QUEUE-CONTRACT.json"
SOURCE_PREVIEW_PATH = REPO_ROOT / "agents" / "project" / "PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-FRESHNESS-AUDIT-PREVIEW.json"

REQUIRED_SOURCE_IDS = {"promotion_asset_owner_decision_evidence_freshness_audit_preview"}
EXPECTED_SOURCE_PATHS = {
    "promotion_asset_owner_decision_evidence_freshness_audit_preview": "agents/project/PROMOTION-ASSET-OWNER-DECISION-EVIDENCE-FRESHNESS-AUDIT-PREVIEW.json",
}

REQUIRED_BOUNDARIES = {
    "local_refresh_queue_contract_only",
...
REQUIRED_SUMMARY_COUNTS = {
    "decision_types": 9,
    "source_freshness_records": 9,
    "refresh_queue_records": 9,
    "queue_states": 5,
...
def load_contract(path: Path = DEFAULT_CONTRACT) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError("promotion asset Owner decision evidence refresh queue contract root must be an object")
    return data
...
def validate_contract(contract: dict[str, Any], repo_root: Path = REPO_ROOT) -> list[str]:
    ...
    if contract.get("$schema") != "autofolio.promotion-asset-owner-decision-evidence-refresh-queue-contract/v1":
        findings.append("unexpected or missing promotion asset Owner decision evidence refresh queue contract schema")

    if contract.get("status") != "local_refresh_queue_contract_not_actual_approval_evidence":
        findings.append("status must be local_refresh_queue_contract_not_actual_approval_evidence")
    ...
    findings.extend(_validate_queue_states(contract))
    findings.extend(_validate_trigger_map(contract))
    findings.extend(_validate_refresh_queue_records(contract))
...
def _validate_queue_states(contract: dict[str, Any]) -> list[str]:
    states = contract.get("queue_states")
    if not isinstance(states, list):
        return ["queue_states must be a list"]
    source = _load_json(SOURCE_PREVIEW_PATH)
    source_states = {
        item.get("state_id"): item
        for item in source.get("state_safety_scan", [])
        if isinstance(item, dict)
    }
    findings: list[str] = []
    missing = set(source_states) - {item.get("state_id") for item in states if isinstance(item, dict)}
    if missing:
        findings.append(f"queue_states missing source state ids: {sorted(missing)}")
    for index, item in enumerate(states):
        if not isinstance(item, dict):
            findings.append(f"queue_states[{index}] must be an object")
            continue
        prefix = f"queue_states[{index}]"
...
def _validate_trigger_map(contract: dict[str, Any]) -> list[str]:
    ...
    for index, item in enumerate(trigger_map):
        ...
        if not isinstance(source_item, dict):
            ...
            continue
        if item.get("target_queue_state") != source_item.get("target_refresh_state"):
            findings.append(f"{prefix}.target_queue_state must match source target_refresh_state")
        for key in ("archive_required", "action_permitted_now"):
...
def _validate_refresh_queue_records(contract: dict[str, Any]) -> list[str]:
    records = contract.get("refresh_queue_records")
    if not isinstance(records, list):
        return ["refresh_queue_records must be a list"]
    source = _load_json(SOURCE_PREVIEW_PATH)
    source_records = {
        item.get("decision_type"): item
        for item in source.get("freshness_record_summaries", [])
        if isinstance(item, dict)
    }
    source_state_ids = {
        item.get("state_id")
    ...
    if missing:
        findings.append(f"refresh_queue_records missing decision types: {sorted(missing)}")
    for index, item in enumerate(records):
        if not isinstance(item, dict):
            findings.append(f"refresh_queue_records[{index}] must be an object")
            continue
        prefix = f"refresh_queue_records[{index}]"
        source_record = source_records.get(item.get("decision_type"))
        if not isinstance(source_record, dict):
            findings.append(f"{prefix}.decision_type unknown")
            continue
        if item.get("source_record_id") != source_record.get("record_id"):
            findings.append(f"{prefix}.source_record_id must match source preview")
        if item.get("current_queue_state") != source_record.get("initial_freshness_state"):
            findings.append(f"{prefix}.current_queue_state must match source initial_freshness_state")
        if item.get("refresh_required_state") != source_record.get("refresh_required_state"):
            findings.append(f"{prefix}.refresh_required_state must match source preview")
        for key in ("expired_state", "future_candidate_state"):
            if item.get(key) not in source_state_ids:
                findings.append(f"{prefix}.{key} must be a known source state")
        if item.get("source_audit_status") != source_record.get("audit_status"):
            findings.append(f"{prefix}.source_audit_status must match source preview")
        ...
        if item.get("source_invalidating_event_count") != source_record.get("invalidating_event_count"):
            findings.append(f"{prefix}.source_invalidating_event_count must match source preview")
        if item.get("stale_trigger_to_refresh_state_count") != source_record.get("stale_trigger_to_refresh_state_count"):
            findings.append(f"{prefix}.stale_trigger_to_refresh_state_count must match source preview")
        ...
        if item.get("queue_status") != "current_local_record_only":
            findings.append(f"{prefix}.queue_status must be current_local_record_only")
    return findings
...
def _validate_events(contract: dict[str, Any]) -> list[str]:
    ...
    for required in (
        "refresh_queue_contract_generated",
        "refresh_queue_records_seeded",
...
def _validate_handoff(contract: dict[str, Any]) -> list[str]:
    ...
    findings: list[str] = []
    for key in ("task_144_contract_ready", "taskset_local_scope_complete_when_task_closes"):
...
def _validate_verification(contract: dict[str, Any]) -> list[str]:
    ...
    if "promotion_asset_owner_decision_evidence_refresh_queue_contract_gate.py --check" not in str(verification.get("local_gate", "")):
...
def main() -> int:
    ...
    if findings:
        print("promotion asset Owner decision evidence refresh queue contract gate: FAIL")
        for finding in findings:
            print(f"- {finding}")
        return 1
    print(f"promotion asset Owner decision evidence refresh queue contract gate: PASS ({args.contract})")
    return 0
