...
from scripts.promotion_asset_owner_decision_evidence_refresh_owner_r3_packet_review_submission_handoff_packet_candidate_archive_rollback_manifest_audit_preview_readiness_index_audit_preview_source_trace_gate import (
    load_trace,
    validate_trace,
)
...
def test_rejects_summary_chain_record_count_drift():
    trace = _trace()
    trace["source_trace_summary"]["chain_records"] = 9

    findings = validate_trace(trace)

    assert any("source_trace_summary.chain_records must be 10" in finding for finding in findings)


def test_rejects_coverage_chain_length_drift():
    trace = _trace()
    trace["source_trace_coverage"]["chain_length"] = 9

    findings = validate_trace(trace)

    assert any("source_trace_coverage.chain_length must be 10" in finding for finding in findings)
...
def test_rejects_missing_source_chain_record():
    ...
    trace["source_chain"] = [
        item for item in trace["source_chain"] if item["trace_record_id"] != "source-trace-handoff-packet-candidate"
    ]

    findings = validate_trace(trace)

    assert any("source_chain must contain 10 records" in finding for finding in findings)
...
def test_rejects_chain_next_relationship_drift():
    ...
    assert any("source_chain[0].expected_source_input_relationship must be audits_source_readiness_index" in finding for finding in findings)
...
def test_rejects_handoff_next_task_drift():
    ...
    assert any("taskset_handoff.next_task_candidate must be TASK-161" in finding for finding in findings)
...
def test_rejects_forbidden_key_name():
    ...
    assert any("forbidden source-trace/submission/approval/signature/export/customer/secret/final-advice key names" in finding for finding in findings)
