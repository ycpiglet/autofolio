...
class TestRecentFills:
    def test_fills_guest_200(self, guest_client, mock_backend):
        resp = guest_client.get("/api/trade/fills/recent")
        assert resp.status_code == 200

    def test_fills_shape(self, guest_client, mock_backend):
        body = guest_client.get("/api/trade/fills/recent").json()
        assert "columns" in body and "rows" in body
        assert "시각" in body["columns"]
        assert "종목" in body["columns"]

    def test_fills_limit_param(self, guest_client, mock_backend):
        resp = guest_client.get("/api/trade/fills/recent?limit=5")
        assert resp.status_code == 200
